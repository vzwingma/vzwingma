<#
.SYNOPSIS
    Generates a distributable ZIP package of the Claude multi-agent templates.
...
.DESCRIPTION
    ...
    Excluded:
      - .github/, .opencode/, scripts/, .git/, .gitignore, .copilotignore
...
.PARAMETER FileName
    Name of the ZIP file (without extension). Defaults to claude-templates-<yyyyMMdd>.

.EXAMPLE
    .\scripts\package-claude.ps1
    .\scripts\package-claude.ps1 -OutputDir C:\tmp\release -FileName claude-templates-v1.0
#>
[CmdletBinding()]
param(
    [string]$OutputDir = '',
...
if ($FileName  -eq '') { $FileName  = "claude-templates-$(Get-Date -Format 'yyyyMMdd')" }

$zipPath = Join-Path $OutputDir "$FileName.zip"

# ── Staging directory ────────────────────────────────────────────────────────
$stagingDir = Join-Path ([System.IO.Path]::GetTempPath()) "claude-pkg-$(Get-Random)"
...
try {
    ...
    Stage-Dir (Join-Path $repoRoot '.claude\agents') '.claude\agents'

    Write-Host "  + .claude/instructions/"
    Stage-Dir (Join-Path $repoRoot '.claude\instructions') '.claude\instructions'

    Write-Host "  + .claude/prompts/"
    $promptsSrc = Join-Path $repoRoot '.claude\prompts'
    if (Test-Path $promptsSrc) {
        Stage-Dir $promptsSrc '.claude\prompts'
    }

    Write-Host "  + .claude/skills/"
    Stage-Dir (Join-Path $repoRoot '.claude\skills') '.claude\skills'

    Write-Host "  + .claude/CHANGELOG.md"
    Stage-File (Join-Path $repoRoot '.claude\CHANGELOG.md') '.claude'

    Write-Host "  + .claude/PLANS.md"
    Stage-File (Join-Path $repoRoot '.claude\PLANS.md') '.claude'
