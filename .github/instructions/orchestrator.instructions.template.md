...
2. **Contexte** : demander aux agents de lire le fichier `.github/instructions/<role>.instructions.md` correspondant.
...
5. **Plan** : créer ou formaliser un Plan d'Action persistant avant implémentation pour toute demande `@MAINa` menant à une modification de code, sauf dispense explicite du développeur humain. La formalisation persistante implique `.github/plans/<NO>_<slug>.plan.md`, `.github/plans/<NO>_reports/` et mise à jour de `.github/plans/README.md`.
...
Formaliser les tâches dans le **Plan d'Action** (`.github/plans/<NO>_<nom>.plan.md`), pas dans une base SQL.

- Une tâche par livrable, assignée à un agent (`🔵 DEVon` / `🟢 QALvin` / `🟣 DOCly`), avec dépendances
  explicites (QA et Doc dépendent du code).
- Chaque agent signale sa complétion via rapport `.github/plans/<NO>_reports/PHASE_N_*.md`.
...
- Ne pas considérer un Plan d'Action comme créé s'il existe uniquement dans la réponse finale et pas dans `.github/plans/`.