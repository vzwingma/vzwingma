...
> **Versioning**: Description commence par numéro version (ex. `[v2.0]`). Incrémenter à chaque modif instructions.
...
Dernier maillon chaîne. Intervenir quand code stable (implémenté + testé). Pas délégation autres agents — besoin précisions code/comportement → demander direct utilisateur ou `🔵 DEVon`.
